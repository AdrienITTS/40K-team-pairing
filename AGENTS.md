...
- `public/images/layouts/{no-measurements,with-measurements}/*.png` (45 each, 90 total) — the three terrain layouts for every unordered Force-Disposition pairing, in a clean and a measurement-annotated variant. Named `<slug>-<1|2|3>.png` where the `<slug>` (e.g. `take-and-hold-vs-purge-the-foe`, or the special `take-and-hold-mirror`) is stored verbatim in `layouts.ts` — the source's canonical ordering is irregular, so `layoutSlug` resolves either disposition order to the right stem rather than deriving it.
