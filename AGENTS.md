...
## Project overview
A Warhammer 40,000 Teams Event companion app (Vue 3 + TypeScript + Vite, `create-vue` scaffold): generates fair pairings between team members, walks through the mission sequence, and tallies VP/BP/TP scoring per the official Teams Event rules. Domain logic and UI are built out: `src/views/` (`HomeView`, `AboutView`, `HowItWorksView`, `PairingView`, `FactionsView`), `src/components/` (`AppFooter`, `CodeWindowCard`, `FactionModal`, and the `pairing/` module — `HandoffGate`, `MatchupCard`, `PairingRunner`, `PairingSetup`), `src/composables/` (`useTheme`, `usePairingSession`), and `src/data/` (`factions.ts`, `pairing.ts`). `src/stores/counter.ts` is an unused scaffold leftover from `create-vue`, not part of the app.
