...
### Pairing Board (Status Badges)

These tokens are **small status badges, not surfaces or brand accents** — the sanctioned exception to the "no cool blue / no fourth surface tone" guidance above. `{colors.role-defender}` is the one deliberately cool-toned token in the system; it's confined to a single small badge glyph on the pairing board and never used as a surface, CTA, or full-bleed accent, so it doesn't compete with coral as brand voltage.

Team identity on the board reuses the existing `{colors.accent-teal}` (Team A) and `{colors.accent-amber}` (Team B) accents — the same team colours used across the rest of the pairing UI — rather than dedicated tokens.
