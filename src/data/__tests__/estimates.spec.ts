...
describe('projectRound', () => {
  ...
  it('picks each table’s column from who declares the layout', () => {
    ...
    expect(p.tables.map((t) => t.quality)).toEqual(['good', 'bad', 'good'])
    expect(p.tables.map((t) => t.source)).toEqual(['defender', 'defender', 'default'])
    expect(p.tables.map((t) => t.bp)).toEqual([18, 6, 10])
    expect(p.ourBp).toBe(34)
    expect(p.oppBp).toBe(26)
  ...
  it('calls the round against the team-size threshold', () => {
    expect(projectRound(matchups, 'A', table, {}, 3).verdict).toBe('win') // +8 ≥ 4
    expect(projectRound(matchups, 'A', table, {}, 7).verdict).toBe('draw') // +8 < 10
    expect(projectRound(matchups, 'A', table, { m2: 'bad' }, 3).verdict).toBe('loss')
  })

  it('awards Team Points for the verdict', () => {
    expect(projectRound(matchups, 'A', table, {}, 3).teamPoints).toBe(3)
    expect(projectRound(matchups, 'A', table, {}, 7).teamPoints).toBe(2)
  ...
  it('reports the BP still needed for a better verdict', () => {
    ...
    const p = projectRound(matchups, 'A', table, {}, 7)
    expect(p.bpToWin).toBe(1)
    expect(p.bpToDraw).toBeNull()
    expect(projectRound(matchups, 'A', table, {}, 3).bpToWin).toBeNull()
  })
